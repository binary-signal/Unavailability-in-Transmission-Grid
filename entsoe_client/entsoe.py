...
class API(object):
    ...
    outage_type = {"Forced": "A54", "Planned": "A53"}

    outage_status = {"Active": "A05", "Cancelled": "A09", "Withdrawn": "A13"}
        ...
        try:
            ...
            try:
                ...
                if "errors" in error_data:
                    logging.error(
                        "post api call bad params " + error_data["errors"][0]["message"]
                    )
        ...
        *,
        ...
        params = (
            ...
            (
                "outageType.values",
                [
                    self.outage_type[param]
                    for param in outage_type
                    if param in self.outage_type
                ],
            ),
            (
                "outageStatus.values",
                [
                    self.outage_status[param]
                    for param in outage_status
                    if param in self.outage_status
                ],
            ),
        )
        ...
        while True:
            ...
            print(f"[1/3] data progress {'{:4d}'.format(prog)}%", end="\r")
            logging.info(f"progress [{have} / {json_data['iTotalRecords']}] " f"data")
    ...
    def data_to_df(data):
        ...
        for d in data:
            ...
            d.update({"dateStart": start_date, "dateEnd": end_date})
    ...
    def parse_data_details(tables_data):
        ...
        if len(tables_data) != 7:
            raise EntsoeApiExcetpion(f"invalid  size for details : " f"{tables_data}")
        return {
    ...
    def curve_grid_unavailability(
        self, detail_id, offset=0, stop_offset=0, batch_size=None, batch_progress=None
    ):
        """
        ...
        while True:
            ...
            msg = f"progress [{have} / {json_curve['iTotalRecords']}] " f"{detail_id}"
    ...
    def details_grid_unavailability_batch(api, detail_id_list):
        ...
        for progress, i in enumerate(detail_id_list):
            try:
                ...
                print(f"[2/3] detail  progress " f"{'{:4d}'.format(prog)}%", end="\r")
    ...
    @staticmethod
    def curve_grid_unavailability_batch(api, detail_id_list, from_date, to_date):
        total = len(detail_id_list)
        have = 0
        timeseries_data = []
        ...
        for progress, i in enumerate(detail_id_list):
            ...
            offset, stop_offset = api.calculate_pagination_offsets(
            ...
            prog = round(100 * ((progress + 1) / total))
            print(f"[3/3] time series  progress " f"{'{:4d}'.format(prog)}%", end="\r")
        print("\n")
    ...
    @staticmethod
    def calculate_pagination_offsets(start_date, end_date, from_date, to_date):
        """
        ...
        if type(start_date) is str and type(end_date) is str:
            start_date = datetime.datetime.strptime(start_date, "%d.%m.%Y %H:%M")
            end_date = datetime.datetime.strptime(end_date, "%d.%m.%Y %H:%M")
        from_date = datetime.datetime.strptime(from_date + " 00:00", "%d.%m.%Y %H:%M")
        to_date = datetime.datetime.strptime(to_date + " 00:00", "%d.%m.%Y %H:%M")
